...
function CyclicOperation({id, name, value, isIncome}) {
    ...
    return (
        <div className="latest-item d-flex align-items-center flex-nowrap justify-content-start">
            ...
            <div className="latest-item__desc d-flex w-100 justify-content-between align-items-center">
                ...
                <span className={ `latest-item__value font-weight-500 w-100 text-right ${ isIncome ? 'color-green' : 'color-red' }` }>{ `${value} zł` }</span>
            </div>
            <div className="resource__close">
                <a href={ `/cyclic/delete/${id}` } className="resources__label d-flex align-items-center justify-content-start color-second" style={{marginLeft:'6px'}}>
